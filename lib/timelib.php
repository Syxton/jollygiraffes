...
function ago($timestamp){
    ...
    if ($difference == 60) {
        ...
        $seconds = floor($difference) > 1 ? $difference . " secs" : $difference . " sec";
    }
        ...
        if (!empty($vars["type"])) {
            ...
            if (!empty($vars["chid"])) {
                if ($child = get_db_row("SELECT * FROM enrollments WHERE pid='$pid' AND chid='" . $vars["chid"] . "'")) {
                    ...
                    $days_array = ["1" => "M", "2" => "T", "3" => "W", "4" => "Th", "5" => "F", "6" => "S", "7" => "Su"];
            ...
            if ($result = get_db_result($SQL)) {
                ...
                while ($row = fetch_row($result)) {
                    ...
                    if ($type == "chid") {
                        ...
                        $content .= get_form($vars["form"], ["month" => $month, "year" => $year, "aid" => $row["aid"],"chid" => $chid,"actid" => $actid,"cid" => $cid,"nid" => $nid,"callback" => "children"], $identifier);
                    ...
                    function(){
                        ...
                        $identifier = $vars["type"] . "_" . $row[$id];
                        $content .= get_form("add_update_employee_activity", ["day" => $list_day, "month" => $month, "year" => $year, "employeeid" => $row["employeeid"],"actid" => $actid,"nid" => $nid,"callback" => "employee"], $identifier);
...
function make_timestamp_from_date($date, $timezone = "UTC") {
    ...
    if (DateTime::createFromFormat('m/d/Y', $date)) {
        ...
        return DateTime::createFromFormat('m-d-Y', $date, $LOCAL)->getTimestamp(); // Noon
    } elseif (DateTime::createFromFormat('Y-m-d\TH:i:s', $date)) {
