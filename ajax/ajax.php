...
global $MYVARS;
    ...
    if($accounts = get_db_result($SQL)){
        while($account = fetch_row($accounts)){
            ...
            $active = get_db_count("SELECT * FROM enrollments WHERE chid IN (SELECT chid FROM children WHERE aid='".$account["aid"]."') AND pid='$pid' AND deleted='$deleted'") ? "activeaccount" : "";
            $deleted_param = $recover ? ',recover: \'true\'':'';
