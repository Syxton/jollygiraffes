...
global $CFG, $MYVARS;
    $type = $MYVARS->GET["type"]; $returnme = $alphabet = $children = ''; $lastinitial = false;
    $pid = get_pid();
    ...
    if($notifications = get_db_result($SQL)){
        while($notification = fetch_row($notifications)){
            ...
            if(!empty($notification["chid"])){
                ...
                $name = get_name(array("type" => "aid", "id" => $aid)) . " Account";
            }else{
                $name = get_name(array("type" => "pid", "id" => $pid));
            }
...
function get_admin_page($type=false,$id=false){
    ...
    $returnme .= get_form("add_edit_account",array("account" => $account),$identifier);
...
global $MYVARS;
    ...
    $returnme = '<div class="container_list scroll-pane ui-corner-all">';
    $tagtypes = array("documents","notes");
    $returnme .= '
        <div class="document_list_item ui-corner-all" style="text-align:center">
...
global $MYVARS;
    ...
    $returnme .= '</div>
                    <div class="list_box_item_right">
                            <div style="width:100px;text-align:center;background:none;line-height: 17px;vertical-align:top;color:white;text-shadow: black 1px 1px 3px;">Only Active Accounts
                                <input type="checkbox" onclick="if($(this).prop(\'checked\')){ $(\'.activeaccount\').hide(); $(\'.inactiveaccounts\').hide(); }else{ $(\'.inactiveaccounts\').show(); } $(\'.scroll-pane\').sbscroller(\'refresh\');" />
                            </div>
    ...
    if($accounts = get_db_result($SQL)){
        while($account = fetch_row($accounts)){
            ...
            $active = get_db_count("SELECT * FROM enrollments WHERE chid IN (SELECT chid FROM children WHERE aid='".$account["aid"]."') AND pid='$pid' AND deleted='$deleted'") ? "activeaccount" : "inactiveaccounts";
            $deleted_param = $recover ? ',recover: \'true\'':'';
