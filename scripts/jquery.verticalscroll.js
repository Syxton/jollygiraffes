...
(function( $ ){
  ...
  $.fn.sbscroller = function( options ) {  
	...
	function setSlider($scrollpane){
        ...
        var contentheight = ($scrollpane.find('.scroll-content').height()+20)
		var difference = contentheight-$scrollpane.height();//eg it's 200px longer
		...
		if(difference>0){ //if the scrollbar is needed, set it up...
			   var proportion = difference / contentheight;//eg 200px/500px
			   ...
			   $scrollpane.find('.slider-vertical').slider({
				  ...
				  change: function(event, ui) {
                     var topValue = -((100-ui.value)*(contentheight-$scrollpane.height())/100);//recalculate the difference on change
					 $scrollpane.find('.scroll-content').css({top:topValue});//move the top up (negative value) by the percentage the slider has been moved times the difference in height
				  }	  
			   });
			
			   //set the handle height and bottom margin so the middle of the handle is in line with the slider
			   $scrollpane.find(".ui-slider-handle").css({height:handleHeight,width:'40px','margin-left':'6px','margin-bottom':-0.5*handleHeight});
			   var origSliderHeight = $scrollpane.height();//read the original slider height
			   var sliderHeight = origSliderHeight - handleHeight ;//the height through which the handle can move needs to be the original height minus the handle height
			   var sliderMargin =  (origSliderHeight - sliderHeight)*0.5;//so the slider needs to have both top and bottom margins equal to half the difference
			   $scrollpane.find(".ui-slider").css({height:(sliderHeight*.96),'margin-top':sliderMargin+5});//set the slider height and margins
